...
export default function StudentStudyDetail() {
  ...
  const fetchCardDetails = async () => {
    ...
    const headers = { Authorization: `Bearer ${token}` };
    try {
      const response = await axios.get(`http://localhost:8000/api/study-material/${id}/`, { headers });
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8000/api/study-material-edit/${editedCard._id}/`,
        cleanedCard,
        { headers }
      );
  ...
  const handleToggleStatus = async (material) => {
    ...
    try {
      await axios.put(`http://localhost:8000/api/study-materials/${material._id}/update/`, { status: updatedStatus });
  ...
  const handleDeleteMaterial = async (id) => {
    ...
    const headers = { Authorization: `Bearer ${token}` };
    try {
      const response = await axios.delete(`http://localhost:8000/api/study-material-delete/${id}/`, { headers });
      if (response.status === 200) {
        navigate(-1);
  ...
  <div className="mt-6">
      ...
      {youtubeLinks.map((link, index) => (
        <button
          ...
          className={`px-4 py-2 rounded-md flex items-center gap-2 ${
            selectedVideo === link ? 'bg-yellow-500 text-white' : 'bg-gray-200 hover:bg-gray-300'
          }`}
        >
          <Play size={16} />
    ...
    {selectedVideo && (
      <div className="flex justify-center">
        <div
          className="w-[480px] h-[270px]" // Smaller fixed size (16:9 ratio)
          style={{
            maxWidth: '100%', // Prevents exceeding parent width
            overflow: 'hidden', // Clips any overflow
            position: 'relative', // Contains the player
          }}
        >
          <ReactPlayer
            url={selectedVideo.link}
            controls
            width="480px" // Matches container width
            height="270px" // Matches container height
            config={{
              youtube: {
                playerVars: {
