...
import placeholderImage from "../../assets/images/internship.png";
...
const InternshipPreview = () => {
  ...
  //       <button
  //         className="absolute top-4 right-4 text-white text-3xl bg-gray-800 rounded-full h-10 w-10 p-2"
  //         onClick={onClose}
  //       >
  //         &times;
  //       </button>
                  ...
                  <div className="w-80 h-80 flex items-center justify-center">
                    {internship.internship_data.image ? (
                      <img
                        ...
                        alt={`${internship.internship_data.company_name} logo`}
                        className="max-w-full max-h-full object-contain rounded-lg"
                        onClick={() => setIsModalOpen(true)}
                        onError={(e) => {
                          e.target.onerror = null;
                          e.target.src = placeholderImage; // Fallback only if image fails to load
                        }}
                      />
                    ) : (
